...
{
    $component_dir = get_stylesheet_directory() . '/components';
    ...
    // Load hero component
    $hero_path = $component_dir . '/hero/hero-universal.php';
    if (file_exists($hero_path)) {
        require_once $hero_path;
    }

    // Load hero solution page component (WorldQuant style)
    $hero_solution_path = $component_dir . '/hero/hero-solution-page.php';
    if (file_exists($hero_solution_path)) {
        require_once $hero_solution_path;
    }

    // Load CTA component
    $cta_path = $component_dir . '/cta/cta-block.php';
    if (file_exists($cta_path)) {
        require_once $cta_path;
    }

    // Load stats component
    $stats_path = $component_dir . '/stats/stats-counter.php';
    if (file_exists($stats_path)) {
        require_once $stats_path;
    }

    // Load testimonial component
    $testimonial_path = $component_dir . '/testimonial/testimonial-carousel.php';
    if (file_exists($testimonial_path)) {
        require_once $testimonial_path;
    }

    // Load Phase 3 Harrison.ai components
    $trust_bar_path = $component_dir . '/trust-bar/trust-bar.php';
    if (file_exists($trust_bar_path)) {
        require_once $trust_bar_path;
    }

    $logo_carousel_path = $component_dir . '/logo-carousel/logo-carousel.php';
    if (file_exists($logo_carousel_path)) {
        require_once $logo_carousel_path;
    }

    $image_composition_path = $component_dir . '/image-composition/image-composition.php';
    if (file_exists($image_composition_path)) {
        require_once $image_composition_path;
    }

    // Load Steps component
    $steps_path = $component_dir . '/steps/steps-block.php';
    if (file_exists($steps_path)) {
        require_once $steps_path;
    }

    // Load Tabs component
    $tabs_path = $component_dir . '/tabs/tabs-block.php';
    if (file_exists($tabs_path)) {
        require_once $tabs_path;
    }

    // Load Gallery component
    $gallery_path = $component_dir . '/gallery/gallery-slider.php';
    if (file_exists($gallery_path)) {
        require_once $gallery_path;
    }

    // Load Problem-Solution component (Phase 2)
    $problem_solution_path = $component_dir . '/problem-solution/problem-solution-block.php';
    if (file_exists($problem_solution_path)) {
        require_once $problem_solution_path;
    }

    // Load Related Pages navigation (Phase 2)
    $related_pages_path = $component_dir . '/navigation/related-pages.php';
    if (file_exists($related_pages_path)) {
        require_once $related_pages_path;
    }
}
