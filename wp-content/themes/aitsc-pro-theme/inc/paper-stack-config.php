...
function aitsc_enqueue_paper_stack_assets() {
    ...
    $supports_scroll_timeline = wp_script_is('aitsc-paper-stack-fallback', 'enqueued');

    // Enqueue fallback JS (only loads if needed via feature detection in JS)
    wp_enqueue_script(
...
function aitsc_load_paper_stack_component() {
    $component_path = get_template_directory() . '/components/paper-stack/paper-stack.php';
